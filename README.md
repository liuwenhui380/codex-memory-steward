...
1. Deterministic scanning
   ...
   Scripts collect verifiable facts first, such as project-root `agent.md` line counts, project-local `.agent/` detail pages, `.agent/project_inventory.md`, `AGENTS.md`, `README.md`, usage markers, and recent session-record locations. This gives the LLM stable input instead of relying on memory or impression.
...
3. Progressive disclosure
   ...
   The project-root `agent.md` stays short and navigation-focused. Detailed operational knowledge lives in project-local `.agent/*.md`, including a generated `project_inventory.md` file tree and rough content map, so future agents can read the most important constraints first and expand into details only when needed.
...
- Direct apply mode: `scripts/run_memory_steward.ps1 -Apply` creates or updates the memory system in the active project folder.

## Memory Location

Place `agent.md` in the target project's root folder. Place detailed memory pages under that same project's `.agent/` directory. Keep `.agent/project_inventory.md` updated with the project file directory and rough file contents. Do not store project memory under `~/.codex`; that directory is only scanned for Codex session records.
